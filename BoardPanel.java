...
public class BoardPanel extends JPanel {
     ...
     * the center).
     */
    public static final int COL_COUNT = 51;

    /**
     * The number of rows on the board. (Should be odd so we can start in
     * the center).
     */
    public static final int ROW_COUNT = 51;
    ...
    public void paintComponent(Graphics g) {
		 ...
		 * where we in relation to the mouse.
		 *
        ...
        if(game.isGameOver() || game.isNewGame() || game.isPaused()) {
            ...
            if(game.isNewGame()) {
                ...
                smallMessage = "Press Enter to Start or press c to watch the Computer play";
    ...
    private void drawTile(int x, int y, TileType type, Graphics g) {
		...
		/*
		 * A mouse is depicted as a small white circle that with a bit of padding
		 * on each side.
		 */
            case Mouse:
                g.setColor(Color.WHITE);
