...
public class SnakeGame extends JFrame {
	...
	/**
	 * The random number generator (used for spawning mice).
	 */
	...
	/**
	 * The number of mice that we've eaten.
	 */
	private int miceEaten;
	
	/**
	 * The number of points that the next mouse will award us.
	 */
	private int nextMouseScore;
	...
	private void updateGame() {
		 ...
		 * 
		 * Mouse: If we collided with a mouse, we increment the number of
		 * mice that we've eaten, update the score, and spawn a new mouse.
		 * 
		 ...
		 * that the next mouse will give us if it's high enough. This adds a
		 * bit of skill to the game as collecting mice more quickly will
		 * yield a higher score.
		 */
		if(collision == TileType.Mouse) {
			miceEaten++;
			score += nextMouseScore;
			spawnMouse();
		} else if(collision == TileType.SnakeBody) {
			isGameOver = true;
			logicTimer.setPaused(true);
		} else if(nextMouseScore > 10) {
			nextMouseScore--;
		}
	}
	...
	private TileType updateSnake() {
		 ...
		 * long enough, and the tile it moved onto is not a mouse.
		 * 
		...
		if(old != TileType.Mouse && snake.size() > MIN_SNAKE_LENGTH) {
	...
	private void resetGame() {
		/*
		 * Reset the score statistics. (Note that nextMousePoints is reset in
		 * the spawnMouse function later on).
		 */
		this.score = 0;
		this.miceEaten = 0;
	...
	private void computerGame(Point head) {
	...
	/**
	 * Spawns a new mouse onto the board.
	 */
	private void spawnMouse() {
		//Reset the score for this mouse to 100.
		this.nextMouseScore = 100;
		...
		for(int x = 0; x < BoardPanel.COL_COUNT; x++) {
			for(int y = 0; y < BoardPanel.ROW_COUNT; y++) {
				...
				if(type == null || type == TileType.Mouse) {
					if(++freeFound == index) {
						board.setTile(x, y, TileType.Mouse);
						 mouseX = x;
						 mouseY = y;
						break;
					}
	...
	/**
	 * Gets the number of mice eaten.
	 * @return The mice eaten.
	 */
	...
	/**
	 * Gets the next mouse score.
	 * @return The next mouse score.
	 */
	public int getNextMouseScore() {
		return nextMouseScore;
	}
